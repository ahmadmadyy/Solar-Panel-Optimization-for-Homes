...
GW.Pop_size = 10;  %the population size
GW.Feasible = 0;
stopping = 0;
...
for gen = 1:GW.Num_Generations  %for the maximum number of generations
    ...
    for i = 4:GW.Pop_size  %for the required number of elites
        GW.Feasible = 0;
        while(GW.Feasible == 0)
            if(abs(GW.A) < 1)
                for j = 1:Panels.num
                    ...
                    x3 = GW.Alpha(1,j) - (abs(GW.A)*D_delta);
                    xAvg(1,j) = (x1 + x2 + x3)/3;
                end
            elseif(abs(GW.A) >= 1)
                for j = 1:Panels.Num
            ...
            for j = 1:Panels.num  %for all the elements in the
                total_Cost = total_Cost + Panels.DB(j,4)*xAvg(i,j)*Gen_newB(i,j); %total cost calculation
                total_Power = total_Power + Panels.DB(j,1)*xAvg(i,j)*Gen_newB(i,j)*0.9; %total energy calculation
                total_Cof = total_Cof + (amb_Temp - 5)*Panels.DB(j,3)*xAvg(i,j)*Gen_newB(i,j); %total temp coeffiecient calculation
                total_Num = total_Num + xAvg(i,j)*Gen_newB(i,j); %total number of panels in current solution calculation
                total_Area = total_Area + xAvg(i,j)*Panels.DB(j,2)*Gen_newB(i,j); %Total area calculation
            end
            area_Waste = (area_Ava-total_Area)/area_Ava; %calculating area wasted percentage
            loss = -(total_Cof/total_Num); %calculating percentage waste due to temp coefficient
            if(total_Cost <= budget && total_Area <= area_Ava && total_Power >= power_Req && loss <= allowable_Loss_Percent && 0 <= area_Waste && area_Waste <= allowable_Waste_Percent) %feasibilty check
